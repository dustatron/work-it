...
export default function ExerciseListSortable({ workoutItems }: Props) {
    ...
    const handleNewSort = (sort: ExerciseInWorkout[]) => {
        const newOrder = exericeList.map((item, index) => ({ id: item?.id, sortOrder: `${index}` }))
        mutate({ newOrder })
        console.log(sort)
    }
    ...
    return (
        <Reorder.Group axis="y" values={exericeList} onReorder={(order) => { setExericeList(order); handleNewSort(order) }}>
            <Stack>

                {exericeList.map((item) => (
                    <Reorder.Item key={item.id} value={item} style={{ listStyle: "none" }}>
                        <span> <ExerciseCard exercise={item.exercise} /></span>
                    </Reorder.Item>
                ))}
            </Stack>
        </Reorder.Group>
