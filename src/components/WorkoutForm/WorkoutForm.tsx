...
export default function WorkoutForm({
  ...
  const { isLoading: isRemoveLoading, mutate: removeThisExercise } =
    api.workout.removeExercise.useMutation({ onSettled: () => refetch() });
  const { isLoading: isAddingLoading, mutate: addExerciseToWorkout } =
    api.workout.addExerciseToWorkout.useMutation({
      onSettled: () => refetch(),
    });
  ...
  const updateFilter = () => {
    ...
    if (region || muscleGroup || formValues.searchTerm) {
      ...
      const filtered = fuse
        .search(`${formValues.searchTerm} ${muscleGroup} ${region}`)
        .map((item) => item.item) as unknown as Exercise[];
  ...
  const removeExercise = (exerciseInWorkoutId: string, exercise: Exercise) => {
    ...
    const isInInitialData = !!initialWorkoutData?.find(
      (item) => item.id === exerciseInWorkoutId,
    );
    ...
    if (isEdit && isInInitialData) {
      removeThisExercise({
        workoutId: workoutId ?? "",
        exerciseInWorkoutId: exerciseInWorkoutId,
      });
    }
  };
  const values = getValues();
  const initialWorkoutDataSorted = initialWorkoutData?.sort(
                  ...
                  <FormControl isInvalid={!!errors.muscleGroup}>
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        setValue(
                          "muscleGroup",
                          e.target.value as MusicGroupType,
                        );
                        updateFilter();
                      }}
                    >
            ...
            <Stack>
              ...
              >
                ...
                <Stack>
                  ...
                  {isAddingLoading && <Skeleton w="100%" h="45px" />}
                </Stack>
              </Card>
